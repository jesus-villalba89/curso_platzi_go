...
func main() {
	...
	wg.Wait() // Funcion del WaitGroup que sirve para decirle al gorutine principal (main) que espere hasta que todas las gorutine del WaitGroup finalicen, es decir, hasta que se ejecute 'defer

	go func(texto string) { // funcion anonima
		fmt.Println(texto)
	}("Adios")
